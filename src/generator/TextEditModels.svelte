...
//    { value: 'gpt-image-1/medium', name: 'GPT-IMAGE-1 medium' },
  ...
  onMount(async () => {
    internalModel = await preference.getOrDefault(model);
  });

  $: if (internalModel !== undefined) {
    model = internalModel;
    preference.set(internalModel).then(() => console.log("save done", internalModel));
  }
  ...
  // 選択されたモードのコスト取得
  $: selectedModel = modelOptionsWithCost.find(option => option.value === internalModel);
  ...
  // 選択処理
  function selectOption(option: {value: TextEditModel, name: string, cost?: number}) {
...
<div class="vbox left gap-2 model">
  ...
  <div class="custom-select-container" on:click|stopPropagation>
    <div class="select-display" on:click={toggleDropdown}>
      {#if selectedModel}
        ...
        <FeathralCost cost={selectedModel.cost} showsLabel={false}/>
      {/if}
    </div>
    
    {#if isOpen}
      <div class="options-container">
        {#each modelOptionsWithCost as option}
          <div
            class="option-item"
            class:selected={option.value === internalModel}
