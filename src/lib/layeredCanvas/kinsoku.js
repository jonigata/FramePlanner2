...
export function kinsoku(overflowDetector, s) {
  let i = 0;
  const getNext = () => i < s.length ? s[i++] : null;
  return Array.from(kinsokuGenerator(overflowDetector, getNext));
}
...
const exampleSentences = [
  ...
  "驚きと喜びが入り混じるニュースを聞いて、彼は目を丸くした。「本当に！？」",
  "彼が尋ねた。「これはどう使うのかな？」｛興味津々な顔で｝",
  "雨が降ってきて、彼女はびしょ濡れになった。「なんてこった！」",
  "サッカーチームは必勝を信じ、練習に励んでいる。「優勝するぞ！」と監督が叫んだ。",
...
exampleSentences.forEach((s) => {
  console.log(s);
  console.log(kinsoku((s) => 1 < s.length, s));
});
